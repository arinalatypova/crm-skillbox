...
export async function createTableBody(
  tableData = "",
...
const createTableItem = (user, body) => {
  ...
  const createContacts = (contacts, container) => {
    ...
    const count =
      contacts.length > maxContactsShows ? maxContactsShows : contacts.length;
  ...
  actions.changeButton.classList.add(
    "table-buttons__button",
    "table-buttons__button_change"
  );
  actions.deleteButton.classList.add(
    "table-buttons__button",
    "table-buttons__button_delete"
  );
  ...
  fullname.innerHTML = `<a class="table-row__elem-link" href="client.html?id=${user.id}">
  ...
  lastUpdateDate.classList.add(
    "table-row__elem",
    "table-row__elem_lastUpdateDate"
  );
