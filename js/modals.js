...
const validateField = (form) => {
  ...
  const validateInputs = (form, contactBlock) => {
    ...
    const surnameError =
      contactBlock.querySelector(".modal__error_surname") ||
      document.createElement("div");
    const firstnameError =
      contactBlock.querySelector(".modal__error_firstname") ||
      document.createElement("div");
  ...
  const validateContact = (contacts, errorBlock) => {
    ...
    const choseSelectType = (selectType, input, contact) => {
      const error =
        errorBlock.querySelector(".modal__error_contact") ||
        document.createElement("div");
      ...
      if (selectType === CONTACT_TYPES.tel) {
        ...
        isValidate = isContactError(
          !re.test(String(input.value).toLowerCase()),
          contact
        );
      } else if (
        selectType === CONTACT_TYPES.vk ||
        selectType === CONTACT_TYPES.facebook
      ) {
  ...
  const errorBlock =
    form.querySelector(".modal__error-block") || document.createElement("div");
...
export async function mainModal(type = TYPE.new, id = "") {
  ...
  // Если меняем существующего пользователя
  if (type === TYPE.change) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (type === TYPE.new) {
      ...
      //Если изменяем существующего пользователя
  ...
  title.innerHTML =
    type === TYPE.new
      ? "Новый клиент"
      : `Изменить данные <span class='modal-title__id'>ID: ${user.id}</span>`;
